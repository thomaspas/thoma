...
- [`docs/CURSOR_REMOTE_EVOX3.md`](docs/CURSOR_REMOTE_EVOX3.md) — Cursor **Desktop** Remote SSH + Simple Browser screen preview (not available to cloud agents)
...
- Screen preview in Cursor (`25_remote_screen_preview.sh`, Simple Browser `:5174`) is **Desktop-only** on the LAN. Cloud agents cannot view the EVO-X3 display.
...
Smoke checks and ports are documented in the runbook (`:11434` LLM, `:8002` bge-m3, `:8000` API, `:5173` UI, `:5174` Cursor screen preview, Docker Postgres `:5432` / Neo4j `:7687`).
...
- Cursor Desktop Remote SSH: [`docs/CURSOR_REMOTE_EVOX3.md`](docs/CURSOR_REMOTE_EVOX3.md). Do not tell the operator that a cloud agent can “see” the kiosk.
