...
PROC_LINES="$(pgrep -af 'ungoogled_chromium|org.chromium|chromium|firefox|evox3-jinhua-kiosk' 2>/dev/null | head -n 20 || true)"
if [ -z "$PROC_LINES" ]; then
  check_fail "No chromium/kiosk process — run ./scripts/evox3/10_relaunch_kiosk.sh"
else
  if printf '%s\n' "$PROC_LINES" | grep -q ":${EVOX3_WEB_PORT}"; then
    check_ok "browser process references :${EVOX3_WEB_PORT}"
  else
